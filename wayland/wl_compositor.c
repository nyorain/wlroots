...
#include <wlr/wayland/wl_compositor.h>
...
static void wl_compositor_destroy(struct wl_resource *wl_resource) {
	struct wlr_wl_compositor *wlr_c = wl_resource_get_user_data(wl_resource);
...
static void wl_compositor_bind(struct wl_client *wl_client,
		void *_wlr_wl_compositor, uint32_t version, uint32_t id) {
	struct wlr_wl_compositor *wlr_c = _wlr_wl_compositor;
...
struct wlr_wl_compositor *wlr_wl_compositor_init(struct wl_display *display) {
	struct wlr_wl_compositor *wlr_c = calloc(1, sizeof(struct wlr_wl_compositor));
